...
public class SendMail {
    ...
    public static boolean sendNewPasswordEmail(String toEmail, String newPassword, String username) {
        try {
            Session session = createEmailSession();
            final String senderEmail = ConfigReader.getProperty("senderEmail");

            Message message = new MimeMessage(session);
            message.setFrom(new InternetAddress(senderEmail, "EagleCam Selection 365"));
            ...
            String htmlContent = "<!DOCTYPE html><html><body>" +
                    ...
                    "<br><p>Trân trọng,<br>EagleCam Selection 365 Team</p>" +
                    "</body></html>";

            message.setContent(htmlContent, "text/html; charset=utf-8");
            Transport.send(message);

            System.out.println("Email mật khẩu mới đã gửi tới: " + toEmail);
            return true;
        } catch (Exception e) {
            e.printStackTrace();
            System.out.println("Email không gửi được");
            return false;
        }
    }
    ...
    public static boolean sendVerificationEmail(String toEmail, String code, String username) {
        try {
            ...
            String verificationLink = "http://localhost:8080/webSource_war/verify?email=" + toEmail + "&code=" + code;
