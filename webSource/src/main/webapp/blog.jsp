...
<section class="ftco-section">
    <div class="container">
        <div class="row">
            <%
                List<model.BlogView> blogs = (List<model.BlogView>) request.getAttribute("blogs");
                if (blogs != null && !blogs.isEmpty()) {
                    java.text.SimpleDateFormat sdf = new java.text.SimpleDateFormat("dd/MM/yyyy");
                    for (model.BlogView b : blogs) {
            %>
            <div class="col-md-4 mb-4">
			    <div class="card h-100 shadow-sm">
			        <div class="blog-image-wrapper">
			            <img src="<%= b.getImageUrl() %>" class="card-img-top blog-image-fixed" alt="Ảnh blog">
			            <div class="blog-overlay">
			                <h6 class="mb-1"><%= b.getUserView().getUserProfile().getFullName() %></h6>
			                <small><%= sdf.format(b.getCreatedAt()) %></small>
			                <hr class="bg-light">
			        ...
			        <div class="card-body">
			            ...
			            <p class="card-text">Ngày đăng: <%= sdf.format(b.getCreatedAt()) %></p>
			            <a href="blog-detail?id=<%= b.getId() %>" class="btn btn-primary">Xem chi tiết</a>
			        </div>
			    </div>
			</div>
            <%
            ...
            %>
            <div class="col-12">
                <p class="text-danger">Không có blog nào được hiển thị.</p>
            </div>
            <%
                }
            %>
...
<script>
	document.addEventListener("DOMContentLoaded", function () {
	    ...
	    const hiddenBlogs = document.querySelectorAll(".blog-item.d-none");
	    let currentIndex = 0;
	    const blogsPerClick = 2;
	
	    loadMoreBtn.addEventListener("click", function () {
	        let count = 0;
	        while (currentIndex < hiddenBlogs.length && count < blogsPerClick) {
	            hiddenBlogs[currentIndex].classList.remove("d-none");
	            hiddenBlogs[currentIndex].classList.add("fade-in");
	            currentIndex++;
	            count++;
	        }
	
	        if (currentIndex >= hiddenBlogs.length) {
